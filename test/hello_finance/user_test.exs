defmodule HelloFinance.UserTest do
  ...
  describe "build/1" do
    test "when all params are valid, returns an User struct" do
      assert {:ok, result} = User.build(@valid_attrs)

      assert %User{name: "Diogo", email: "dcdourado@gmail.com", password: "123456"} = result
    end

    test "when a required param is not inserted, returns an error" do
      assert {:error, no_name_result} = User.build(@no_name_attrs)

      assert %Changeset{valid?: false} = no_name_result
      assert errors_on(no_name_result) == %{name: ["can't be blank"]}

      assert {:error, no_email_result} = User.build(@no_email_attrs)

      assert %Changeset{valid?: false} = no_email_result
      assert errors_on(no_email_result) == %{email: ["can't be blank"]}

      assert {:error, no_password_result} = User.build(@no_password_attrs)

      assert %Changeset{valid?: false} = no_password_result
    ...
    test "when password do not attend min length (6), returns an error" do
      assert {:error, result} = User.build(@invalid_password_attrs)
    ...
    test "when email has invalid format, returns an error" do
      assert {:error, result} = User.build(@invalid_email_attrs)
    ...
    test "when email has already been taken, returns an error" do
      assert {:ok, user} = User.build(@valid_attrs)

      Repo.insert(user)

      assert {:error, result} = User.build(@valid_attrs)
